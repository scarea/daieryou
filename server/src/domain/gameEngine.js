...
function calculateRound(gameState) {
  ...
  const publicCard = gameState.publicCards[publicCardIndex]
  const isHiddenCard = currentRound === 4

  const evaluations = gameState.players.map((player, playerIndex) => {
    const selectedHandCards = player.selectedCards.map((index) => player.handCards[index])
    const hand = [...selectedHandCards, publicCard]
    return { playerIndex, hand, evaluation: evaluateHand(hand) }
  })

  evaluations.sort((left, right) => compareHands(right.evaluation, left.evaluation))

  const loserIndex = evaluations[1].playerIndex
  const winnerIndex = evaluations[0].playerIndex
  const lastIndex = evaluations[2].playerIndex
  const roundScore = gameConfig.roundScores[currentRound - 1]

  gameState.players[winnerIndex].roundScores.push(roundScore)
  gameState.players[winnerIndex].totalScore += roundScore
  gameState.players[loserIndex].roundScores.push(-roundScore)
  gameState.players[loserIndex].totalScore -= roundScore
  gameState.players[lastIndex].roundScores.push(0)

  const roundResult = {
    round: currentRound,
    publicCard: isHiddenCard ? null : publicCard,
    playerResults: evaluations.map((entry, index) => ({
...
function prepareNextRound(gameState, loserIndex) {
  ...
  if (gameState.currentRound !== 2 && gameState.currentRound !== 3) {
    return
  }

  const drawOrder = [loserIndex]
  for (let playerIndex = 0; playerIndex < gameState.players.length; playerIndex += 1) {
    if (playerIndex !== loserIndex) {
      drawOrder.push(playerIndex)
    }
  }

  for (let round = 0; round < 2; round += 1) {
    drawOrder.forEach((playerIndex) => {
